...
class RemoteTradeService implements TradeService {
  ...
  @AssistedInject
  RemoteTradeService(RemoteMarketDataConfiguration configuration, Client client, @Assisted String exchange) {
  ...
  private String placeOrder(ExchangeResource.OrderPrototype order) {
    Response response = client.target(String.format("%s/%s/orders", configuration.getExchangeEndpointUri(), exchange))
        .request()
    ...
    if (response.getStatus() == 200) {
      ...
      ExchangeResource.ErrorResponse errorResponse = response.readEntity(ExchangeResource.ErrorResponse.class);
  ...
  public boolean cancelOrder(CancelOrderParams orderParams) throws IOException {
    ...
    Response response = client.target(
        String.format("%s/%s/markets/%s-%s/orders/%s",
