...
class JobLockerImpl implements JobLocker, Managed {
  ...
  @Inject
  JobLockerImpl(JobRunConfiguration configuration,
  ...
  boolean attemptLock(String jobId, UUID uuid, LocalDateTime dateTime) {
    try {
      ...
      boolean result = connectionSource.getWithCurrentConnection(dsl ->
        dsl.insertInto(TABLE)
        .values(jobId, uuid.toString(), newExpiryDate(dateTime)).execute()) == 1;
      if (result) {
  ...
  boolean updateLock(String jobId, UUID uuid, LocalDateTime dateTime) {
    ...
    return connectionSource.getWithCurrentConnection(dsl -> dsl.update(TABLE).set(EXPIRES_FIELD, newExpiryDate(dateTime)).where(fullKeyMatch(jobId, uuid)).execute()) != 0;
  }
  ...
  public void releaseLock(String jobId, UUID uuid) {
    ...
    connectionSource.withCurrentConnection(dsl -> dsl.delete(TABLE).where(fullKeyMatch(jobId, uuid)).execute());
  }
  ...
  public void releaseAnyLock(String jobId) {
    connectionSource.withCurrentConnection(dsl -> dsl.delete(TABLE).where(JOB_ID_FIELD.eq(jobId)).execute());
  }
